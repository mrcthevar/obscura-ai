...
const Toast: React.FC<ToastProps> = ({ message, type = 'error', onClose, duration = 5000 }) => {
  ...
  return (
    <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] transition-all duration-300 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
      <div className={`backdrop-blur-xl border px-6 py-4 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] flex items-center gap-4 ${bgColors[type]}`}>
        <div className="flex flex-col">
           ...
           <span className="font-mono text-xs font-bold whitespace-nowrap">{message}</span>
        </div>
        <button onClick={() => setIsVisible(false)} className="opacity-50 hover:opacity-100 transition-opacity p-1">
