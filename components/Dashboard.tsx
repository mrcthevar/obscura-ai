...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout }) => {
  ...
  const handleCreateNewSave = async (name: string) => {
    ...
    try {
      ...
      console.error(e);
      alert("Failed to save project to Drive. Ensure popups are allowed.");
    } finally {
  ...
  const handleLoadList = async () => {
    ...
    try {
        ...
        console.error(e);
        // Force re-auth if token is invalid
        handleConnectDrive();
    } finally {
        setIsLoadingDrive(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-200">
      {/* Sidebar / Navigation */}
      <nav className="fixed top-0 left-0 h-full w-20 md:w-64 border-r border-neutral-900 bg-[#050505] z-40 hidden md:flex flex-col">
        ...
        <div className="flex-1 py-8 space-y-2 overflow-y-auto">
          {/* Project Controls */}
          <div className="px-8 pb-4 border-b border-neutral-900 mb-4 space-y-3">
             <div className="text-[10px] text-neutral-600 font-bold uppercase tracking-widest mb-2">Project Database</div>
             
             {!isDriveConnected ? (
               <button onClick={handleConnectDrive} className="w-full flex items-center justify-center space-x-2 bg-[#0A0A0A] border border-neutral-800 py-2 hover:border-[#FFD700] text-xs text-neutral-400 hover:text-white transition-all">
               ...
               <>
                 <button 
                   ...
                   className="w-full text-left flex items-center space-x-3 text-neutral-400 hover:text-[#FFD700] transition-colors text-xs font-inter"
                 >
          ...
          <button
            onClick={() => setCurrentView('HOME')}
            className={`w-full text-left px-8 py-4 flex items-center space-x-4 transition-all duration-300 ${
              currentView === 'HOME' 
          ...
          <div className="px-8 py-2 text-[10px] text-neutral-600 font-bold uppercase tracking-widest mt-4 mb-1">Modules</div>

          {MODULES.map((mod) => (
            <button
              key={mod.id}
              onClick={() => setCurrentView(mod.id)}
              className={`w-full text-left px-8 py-3 flex items-center space-x-4 transition-all duration-300 ${
                currentView === mod.id 
                  ? 'bg-[#0A0A0A] text-[#FFD700] border-r-2 border-[#FFD700]' 
                  : 'text-neutral-500 hover:text-white'
              }`}
          ...
          <div className="px-8 py-2 text-[10px] text-neutral-600 font-bold uppercase tracking-widest mt-6 mb-1">Resources</div>
          
          <button onClick={() => setCurrentView('GUIDE')} className={`w-full text-left px-8 py-3 flex items-center space-x-4 transition-all duration-300 ${currentView === 'GUIDE' ? 'text-[#FFD700]' : 'text-neutral-500 hover:text-white'}`}>
          ...
          <button onClick={() => setCurrentView('FAQ')} className={`w-full text-left px-8 py-3 flex items-center space-x-4 transition-all duration-300 ${currentView === 'FAQ' ? 'text-[#FFD700]' : 'text-neutral-500 hover:text-white'}`}>
          ...
          <button onClick={() => setCurrentView('SUPPORT')} className={`w-full text-left px-8 py-3 flex items-center space-x-4 transition-all duration-300 ${currentView === 'SUPPORT' ? 'text-[#FFD700]' : 'text-neutral-500 hover:text-white'}`}>
        ...
        <div className="p-8 border-t border-neutral-900 flex items-center gap-4">
      ...
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#050505] border-b border-neutral-900 z-40 flex items-center justify-between px-4">
          <button onClick={() => setCurrentView('HOME')} className="font-cinzel text-white hover:text-[#FFD700] transition-colors">OBSCURA</button>
          {currentView !== 'HOME' && (
            <button onClick={() => setCurrentView('HOME')} className="text-sm text-neutral-400 flex items-center gap-1">
      ...
      {/* Main Content Area */}
      <main className="md:ml-64 p-6 md:p-12 pt-20 md:pt-12 min-h-screen relative bg-[#050505]">
        {isLoadingDrive && (
        ...
        {currentView === 'HOME' && (
          <div className="max-w-6xl mx-auto animate-fade-in">
            <h2 className="text-4xl md:text-5xl font-cinzel text-white mb-12">Cinematic Intelligence Suite</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {MODULES.map((mod) => (
                <button
                  key={mod.id}
                  onClick={() => setCurrentView(mod.id)}
                  className="group bg-[#0A0A0A] border border-neutral-800 p-8 text-left hover:border-[#FFD700] hover:shadow-[0_0_30px_rgba(255,215,0,0.1)] transition-all duration-500 relative overflow-hidden"
                >
                  <div className="h-12 w-12 bg-neutral-900 rounded-full flex items-center justify-center text-[#FFD700] mb-6 group-hover:scale-110 transition-transform">
                  ...
                  <h3 className="text-xl font-cinzel text-white mb-2">{mod.title}</h3>
                  <p className="text-base text-neutral-400 font-inter leading-relaxed group-hover:text-neutral-300 transition-colors">{mod.description}</p>
                </button>
              ))}
            </div>
          </div>
      ...
      {showLoadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#0A0A0A] border border-neutral-700 w-full max-w-2xl p-8 max-h-[80vh] overflow-hidden flex flex-col shadow-[0_0_40px_rgba(255,215,0,0.1)] rounded relative">
             <div className="flex justify-between items-center mb-6 pb-4 border-b border-neutral-800">
                <h3 className="text-2xl font-cinzel text-white">Load Project</h3>
                <button onClick={() => setShowLoadModal(false)} className="text-neutral-500 hover:text-white">✕</button>
             </div>
                ...
                {projectList.length === 0 ? (
                    ...
                    projectList.map(file => (
                        <button 
                           ...
                           className="w-full text-left p-4 bg-[#050505] border border-neutral-800 hover:border-[#FFD700] transition-colors flex justify-between items-center group"
                        >
                           <div>
                               <p className="text-white font-mono text-sm group-hover:text-[#FFD700]">{file.name.replace('.json', '')}</p>
                               <p className="text-neutral-600 text-[10px] mt-1">ID: {file.id}</p>
                           </div>
                           <span className="text-neutral-500 text-xs font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100">LOAD</span>
                        </button>
                    ))
                )}
             </div>
